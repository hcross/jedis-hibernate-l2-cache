...
public class JedisCacheManager implements CacheManager {
    ...
    public JedisCacheManager(Map<String, Object> properties) {
        if (properties.keySet().stream().anyMatch(key -> key.startsWith(SENTINEL_CONFIG_PREFIX))) {
        ...
        if (properties.keySet().stream().anyMatch(key -> key.startsWith(STANDALONE_CONFIG_PREFIX))) {
